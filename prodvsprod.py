...
- Latest dump selected by NAME date token (not mtime).
- Config interpolation disabled (handles %20 safely).
- XLSX-only outputs (no CSV anywhere). If XLSX writing fails -> email + exit(6).
- SharePoint link forced to .xlsx and "(Execution Date)" removed.
- codecomp.html:
    * Pretty page written at run root: /YYYY-MM-DD/codecomp.html
    * Links are deterministic: <old>_vs_<new>/codecomp/<meta>/<meta>.html
    * The file inside .../<old>_vs_<new>/codecomp/codecomp.html is a redirect to ../../codecomp.html
- If no new dump -> email + marker file + exit(7) (visible no-op).
"""
...
def chips_html(items) -> str:
    return ('<div class="chips">' + "".join(f'<span class="chip meta">{_html.escape(x)}</span>' for x in items) + '</div>') if items else '<div class="note">No metadata configured.</div>'
...
# =========================
# codecomp.html builder
# =========================
def human_from_label(name: str) -> str:
...
def beautify_codecomp_html(codecomp_path: Path,
                           ...
                           new_counts: Counter,          # per top-level dir
                           modified_counts: Counter,     # per top-level dir
                           prev_human: str = None,
                           latest_human: str = None):
    """
    Build a minimal Diff-Only page (no BC raw dependency), with deterministic links:
      href = "<old>_vs_<new>/codecomp/<meta>/<meta>.html"
    Pretty page is written at run root: out_copy_at_run_root (required).
    codecomp_path (inside .../codecomp/) becomes a redirect to "../../codecomp.html"
    """

    # Friendly badges
    lhs_badge = prev_human or human_from_label(old_label)
    rhs_badge = latest_human or human_from_label(new_label)

    # Deterministic per-type links from the run root
    pair_dir = f"{old_label}_vs_{new_label}/codecomp"

    # Normalize keys
    n_map = {str(k).strip(): int(v) for k, v in (new_counts or {}).items()}
    m_map = {str(k).strip(): int(v) for k, v in (modified_counts or {}).items()}
    metas = sorted(set(n_map.keys()) | set(m_map.keys()), key=lambda s: s.lower())

    def row(meta: str) -> str:
        href = f"{pair_dir}/{meta}/{meta}.html"
        new_n = n_map.get(meta, 0)
        mod_n = m_map.get(meta, 0)
        tot_n = new_n + mod_n
        label = _html.escape(meta)
        return (
            f"<tr>"
    ...
    rows_html = "\n".join(row(m) for m in metas) if metas else (
        '<tr><td colspan="4" class="empty">No differences detected.</td></tr>'
    )

    # Pretty page (run root)
...
<style>
  ...
  *,*::before,*::after{{box-sizing:border-box}}
  ...
  .num.black{{color:#111}} .num.green{{color:var(--green)}} .num.orange{{color:#d97706}}
...
<header>
  <div class="h-inner">
    <div class="h-title">
      ...
      <div class="badges">
        <span class="badge">LHS (Previous): <strong>{_html.escape(lhs_badge)}</strong></span>
        <span class="badge">vs</span>
        <span class="badge">RHS (Latest): <strong>{_html.escape(rhs_badge)}</strong></span>
      </div>
    </div>
    <div class="brand">
...
"""
    ...
    # Write pretty page at run root
    if not out_copy_at_run_root:
        raise RuntimeError("out_copy_at_run_root is required for pretty codecomp page.")
    out_copy_at_run_root.parent.mkdir(parents=True, exist_ok=True)
    write_text(out_copy_at_run_root, pretty_html)

    # Write redirect page inside .../<old>_vs_<new>/codecomp/
    codecomp_path.parent.mkdir(parents=True, exist_ok=True)
    redirect_html = """<!DOCTYPE html>
<html><head>
<meta http-equiv="refresh" content="0; url=../../codecomp.html">
<title>Redirecting…</title>
</head>
<body>Redirecting to the main diff summary…</body></html>"""
    write_text(codecomp_path, redirect_html)
...
def main():
    ...
    prev_human   = human_from_label(old_src.name) if not d_old else f"{ordinal(d_old.day)} {MONTH_LONG[d_old.month]} {d_old.year}"
    latest_human = human_from_label(new_src.name) if not d_new else f"{ordinal(d_new.day)} {MONTH_LONG[d_new.month]} {d_new.year}"
    ...
    cnt_rm  = Counter(top_folder(r) for r in removed_rel)  # (not used in diff-only table, but kept for completeness)
    ...
    # Beautify codecomp.html (run root pretty + codecomp redirect)
    bc_out = run / f"{old_src.name}_vs_{new_src.name}" / "codecomp" / "codecomp.html"
    bc_alias = run / "codecomp.html"  # pretty page location
    try:
        beautify_codecomp_html(
            codecomp_path=bc_out,
            out_copy_at_run_root=bc_alias,
            old_label=old_src.name,
            new_label=new_src.name,
            new_counts=cnt_new,
            modified_counts=cnt_chg,
            prev_human=prev_human,
            latest_human=latest_human
        )
        append_text(log, "Built codecomp.html (pretty) + redirect\n")
    except Exception as e:
        append_text(log, f"[WARN] codecomp build failed: {e}\n")
